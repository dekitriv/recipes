...
namespace Recipes.Implementation.Validators
{
    ...
    {
        public UpdateRecipeValidator(RecipesContext context)
        {
            RuleFor(x => x.Name)
                .NotEmpty()
                ...
                {
                    RuleFor(x => x.Name)
                    .Must((dto, name) => !context.Recipes.Any(x => x.Name == name && dto.Id == x.Id))
                    .WithMessage(dto => $"Recipe with name {dto.Name} already exists");
                });
